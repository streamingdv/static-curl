...
- Features: alt-svc asyn-rr AsynchDNS brotli ECH HSTS HTTP2 HTTP3 HTTPS-proxy HTTPSRR IPv6 Largefile libz NTLM PSL SSL SSLS-EXPORT threadsafe TLS-SRP UnixSockets zstd

This fork links neither `libidn2` nor `libunistring`, both of which are LGPL, so the binary can be
redistributed without LGPL obligations. The cost is that `IDN` is absent from the Linux and macOS
builds; the Windows builds still report it, because they use `winidn` from the operating system.
